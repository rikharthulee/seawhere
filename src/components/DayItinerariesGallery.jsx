...
export default function DayItinerariesGallery({
  rows = [],
  basePath = "/day-itineraries",
}) {
  ...
  return (
    <section id="day-itineraries">
      <div className="pt-2">
        <div className="flex items-end justify-between">
          <h2 className="text-3xl md:text-4xl font-medium">Day Itineraries</h2>
        </div>
      </div>
      ...
      {sorted.length === 0 ? (
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 md:grid-cols-3">
          {sorted.map((dayItinerary) => {
            const slug = dayItinerary.slug;
            const imageSrc =
              resolveImageUrl(dayItinerary.cover_image) ||
              placeholderDataUrl(dayItinerary.name || dayItinerary.title);
            return (
              <Tile.Link
                key={dayItinerary.id}
              ...
              >
                <Tile.Image>
                  <SafeImage
                    src={imageSrc}
                    alt={dayItinerary.title || dayItinerary.name}
                    fill
                ...
                <Tile.Content>
                  <div className="font-medium">
                    {dayItinerary.title || dayItinerary.name}
                  </div>
