...
export default function POIsManager() {
  ...
  async function load() {
    ...
    setError("");
    try {
      const res = await fetch("/api/admin/pois", { cache: "no-store" });
      const json = await res.json();
      if (!res.ok) throw new Error(json?.error || `HTTP ${res.status}`);
      const list = json.items || [];
      setItems(list);
      // Fetch human-readable destination names via admin meta endpoint
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded bg-black text-white px-3 py-2" onClick={() => setEditing({})}>+ New POI</button>
      </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td className="px-3 py-3" colSpan={5}>No POIs. Click “New POI” to create your first one.</td>
              </tr>
            ) : (
              items.map((it) => {
                return (
                  <tr key={it.id} className="border-t">
                    ...
                    <td className="px-3 py-2 text-right">
                      ...
                      {destMap[it.destination_id]?.slug ? (
                        <a className="rounded border px-2 py-1 mr-2 inline-block" href={`/destinations/${encodeURIComponent(destMap[it.destination_id].slug)}`} target="_blank" rel="noopener noreferrer">View</a>
                      ) : null}
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            const res = await fetch(`/api/admin/pois/${it.id}`, { method: "DELETE" });
                            if (!res.ok) {
