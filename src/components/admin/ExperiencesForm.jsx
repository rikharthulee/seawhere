...
export default function ExperiencesForm({ id, initial, onSaved, onCancel }) {
  ...
  const [priceAmount, setPriceAmount] = useState(initial?.price_amount ?? initial?.price?.amount ?? "");
  const [priceCurrency, setPriceCurrency] = useState(initial?.price_currency || initial?.price?.currency || "USD");
  ...
  useEffect(() => {
    ...
    setLat(initial?.lat ?? "");
    setLng(initial?.lng ?? "");
    setPriceAmount(initial?.price_amount ?? initial?.price?.amount ?? "");
    setPriceCurrency(initial?.price_currency || initial?.price?.currency || "USD");
  ...
  const experienceUploadSlug = slugify(slug || name) || (initial?.id ? `id-${initial.id}` : "unsorted");
  ...
  async function save() {
    try {
      ...
      let json;
      if (isEditing) {
        res = await fetch(`/api/admin/experiences/${id}`, {
          method: "PUT",
      ...
      try {
        await fetch(`/api/revalidate`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ tags: ["experiences", `experiences:${savedSlug}`] }),
        });
      } catch {}
      onSaved?.();
    } catch (e) {
  ...
  return (
    <Card>
      <CardContent className="space-y-4">
        ...
        <MultiImageUpload label="Images" value={images} onChange={setImages} prefix={experienceUploadPrefix} />
