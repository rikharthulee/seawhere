...
export function buildTransportInsertRows({ excursionId, legs = [], items = [] }) {
  if (!excursionId || !Array.isArray(legs) || legs.length === 0) return [];
  ...
  return legs
    .map((leg) => {
      ...
      return {
        ...
        summary: leg.summary || (typeof leg.details === 'string' ? leg.details : null),
        steps: stepsPayload,
