...
export async function getAccommodationBySlugPublic(slug) {
  ...
  const { data, error } = await db
    .from("accommodation")
    .select(
      "slug, name, summary, description, images, status, credit, price_band, rating, website_url, affiliate_url, lat, lng, address, geocoded_address, google_formatted_address, google_photos, destination_id, country_id"
    )
    .eq("slug", slug)
    .eq("status", "published")
    .maybeSingle();
