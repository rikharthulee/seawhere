...
export async function getFoodDrinkBySlugPublic(slug) {
  ...
  const { data, error } = await db
    .from("food_drink")
    .select(
      "slug, name, description, images, status, type, price_band, rating, booking_url, address, geocoded_address, google_formatted_address, google_photos, lat, lng, destination_id, country_id"
    )
    .eq("slug", normalized)
    .eq("status", "published")
    .maybeSingle();
