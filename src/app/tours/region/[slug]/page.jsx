...
export default async function ToursByRegionPage({ params }) {
      ...
      <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {Array.isArray(items) && items.length > 0 ? (
          items.map((p) => {
            ...
            const linkProps = canLink ? { href: `/tours/${encodeURIComponent(destSlug)}/${encodeURIComponent(p.slug)}` } : {};
            return (
              <Tag
                key={p.id}
                {...linkProps}
          ...
          <div className="col-span-full text-black/60">No tours found for this region.</div>
        )}
      </section>
    </main>
  );
