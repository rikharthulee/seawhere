    ...
    <main className="mx-auto max-w-4xl px-4 py-10 space-y-8">
      <header className="space-y-3">
        ...
        <div className="flex flex-wrap gap-2">
          {[...Array(4)].map((_, idx) => (
            <Skeleton key={idx} className="h-6 w-20 rounded-full bg-muted/40" aria-hidden />
          ))}
        </div>
      </header>
      ...
      <section className="space-y-3">
        ...
        {[...Array(4)].map((_, idx) => (
          <div key={idx} className="rounded-xl border border-border/60 bg-card/40 p-4 space-y-3">
            <div className="flex items-start gap-4">
              <Skeleton className="h-16 w-16 rounded-full bg-muted/50" aria-hidden />
