...
function normalizeDayItineraries(rows = []) {
  return rows
    ...
    .map((row) => {
      ...
      return {
        id: row.id,
        title: row.title || row.name || "Untitled day itinerary",
        ...
        hasTransport: (() => {
          const relationCount =
            Array.isArray(row.day_itinerary_transport_legs) &&
            row.day_itinerary_transport_legs.length > 0
              ? Number(row.day_itinerary_transport_legs[0]?.count || 0)
              : 0;
...
function dayItineraryMeta(dayItinerary) {
  const meta = [];
  if (dayItinerary.status) {
    meta.push(
      dayItinerary.status === "published"
        ? "Published"
        : dayItinerary.status.charAt(0).toUpperCase() +
          dayItinerary.status.slice(1)
    );
  }
  if (dayItinerary.hasTransport) {
    meta.push("Includes transport");
  }
  if (dayItinerary.hasMap) {
    meta.push("Map link");
  }
  return meta.join(" • ");
}

function DayItineraryCard({ itinerary }) {
  const meta = dayItineraryMeta(itinerary);
  const href = `/admin/itineraries/builder?id=${encodeURIComponent(
    itinerary.id
  )}`;
  const updatedDate = itinerary.updatedAt
...
function CreateDayItineraryCard() {
  return (
    <Card
      asChild
    ...
    >
      ...
      >
        ...
        <div className="space-y-2">
          <h3 className="text-lg font-semibold">Create new day itinerary</h3>
          <p className="text-sm text-muted-foreground">
            Start from a blank canvas and build a reusable day itinerary for
            your trips.
          </p>
        </div>
      </Link>
...
export default async function AdminItinerariesIndex() {
  let rows = [];
  ...
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin/itineraries?limit=200`, {
      cache: "no-store",
  ...
  const itineraries = normalizeDayItineraries(rows);
    ...
    <main className="mx-auto max-w-6xl px-4 py-10">
      <header className="max-w-3xl space-y-3">
        ...
        <p className="text-base text-muted-foreground sm:text-lg">
          Jump into an existing day itinerary to tweak it or spin up a fresh
          template for your clients.
        </p>
      </header>

      <section className="mt-10">
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          <CreateDayItineraryCard />
          {itineraries.map((itinerary) => (
            <DayItineraryCard key={itinerary.id} itinerary={itinerary} />
          ))}
        </div>
