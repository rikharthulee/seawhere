...
import { listExperiencesByPrefectureSlug } from "@/lib/data/public/experiences";
...
export default async function ExperiencesByPrefecturePage(props) {
  ...
  if (!pref?.id) notFound();
  const items = await listExperiencesByPrefectureSlug(slug);
      ...
      <section className="mt-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {Array.isArray(items) && items.length > 0 ? (
          ...
          <div className="col-span-full text-black/60">
            No experiences found for this prefecture.
          </div>
        )}
      </section>
    </main>
